...
- **Vercel Postgres** (Neon) - serverless Postgres database
...
This app is designed to deploy on **Vercel** with **Neon Postgres**.
...
2. Add a **Neon Postgres** database via Vercel Storage (Marketplace) — this auto-sets `POSTGRES_URL`
3. Add the remaining environment variables (see below)
4. Deploy
...
3. Create **OAuth consent screen** (Internal for Google Workspace)
...
4. Authorize with your Google Workspace account and exchange for tokens
...
|----------|-------------|
| `POSTGRES_URL` | Neon Postgres connection string (auto-set by Vercel if using marketplace) |
| `GOOGLE_CLIENT_ID` | Google OAuth2 client ID |
| `GOOGLE_CLIENT_SECRET` | Google OAuth2 client secret |
| `GOOGLE_REFRESH_TOKEN` | OAuth2 refresh token (Gmail + Drive scopes) |
| `GMAIL_FROM` | Email address to send verification emails from |
| `GOOGLE_DRIVE_FOLDER_ID` | Google Drive folder ID for signed PDFs |
| `ADMIN_PASSWORD` | Password to access the admin panel at `/admin` |

### Admin Password

Set `ADMIN_PASSWORD` in `.env.local` or Vercel env vars. Access the admin panel at `/admin`.
...
src/
  ...
  lib/
    db.ts                       # Postgres schema, queries, team management
